...
export default function PromoBannerForm() {
  ...
  const bannerQuery = useQuery({
    ...
    queryFn: async () =>
      (await api.get(`/admin/promo-banners/${id}`)).data.banner,
  });
  ...
  const save = useMutation({
    mutationFn: async () => {
      if (isNew)
        return (await api.post("/admin/promo-banners", payload)).data.banner;
  ...
  const uploadImage = useMutation({
    ...
    onError: (err) => {
      ...
      const msg =
        ...
        (status ? `Upload gagal (HTTP ${status})` : "Upload gagal");
      const errors = err?.response?.data?.errors;
      const extra = errors?.image?.[0];
      setImageError(extra ? `${msg}: ${extra}` : msg);
    },
  });

  const deleteImage = useMutation({
    mutationFn: async () =>
      (await api.delete(`/admin/promo-banners/${id}/image`)).data,
    onSuccess: () => {
      setForm((f) => ({ ...f, image: "" }));
      setImageFile(null);
      if (imagePreview) URL.revokeObjectURL(imagePreview);
        ...
        <CardContent className="grid gap-3 md:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              value={form.kicker}
              onChange={(e) =>
                setForm((f) => ({ ...f, kicker: e.target.value }))
              }
            />
          </div>
            ...
            <div className="flex flex-1 flex-wrap gap-2">
              ...
              {isNew ? (
                <div className="w-full text-xs text-zinc-500">
                  Save banner first to upload image.
                </div>
              ) : null}
              {imageError ? (
                <div className="w-full text-xs text-red-500">{imageError}</div>
              ) : null}
            </div>
          </div>
        </CardContent>
