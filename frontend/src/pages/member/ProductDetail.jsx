...
function formatNutritionKey(key) {
  ...
  if (k.endsWith("_kcal"))
    return `${k.replace(/_kcal$/, "").replace(/_/g, " ")} (kcal)`;
  if (k.endsWith("_mg"))
    return `${k.replace(/_mg$/, "").replace(/_/g, " ")} (mg)`;
...
function hasCompleteModifiers(product, selectedIds) {
  ...
  return modifiers.every((m) =>
    (m.options ?? []).some((opt) => selected.has(opt.id)),
  );
}
...
export default function ProductDetail({ basePath = "/member" }) {
  ...
  return (
    <div className="space-y-6 px-4">
      <div className="aspect-video overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-950">
        {imageSrc ? (
          <img
            src={imageSrc}
            alt={p.name}
      ...
      {p.description ? (
        <div className="rounded-3xl border border-zinc-800 bg-linear-to-br from-emerald-900 to-zinc-950/50 p-4 text-zinc-300">
          {p.description}
        </div>
      ) : null}

      {nutritionEntries.length ? (
        <div className="rounded-3xl border border-zinc-800 bg-linear-to-br from-emerald-900 via-emerald-950 to-zinc-950/50 p-4">
          <div className="font-medium">Nutrition</div>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {nutritionEntries.map((n) => (
              <div
                key={n.key}
                className="flex items-center justify-between rounded-3xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm"
              >
                <div className="capitalize text-zinc-400">
                  {formatNutritionKey(n.key)}
                </div>
      ...
      {Array.isArray(p.modifiers) && p.modifiers.length ? (
        <div className="text-sm text-amber-300">
          Pilih pilihan/varian terlebih dulu sebelum tambah ke cart.
        </div>
      ) : null}

      <div className="flex flex-col gap-3 rounded-3xl border border-zinc-800 bg-zinc-900 p-4 sm:flex-row sm:items-center">
        ...
        <button
          type="button"
          className={[
            "flex-1 rounded-3xl px-4 py-2 font-medium",
            canAddToCart
              ? "bg-linear-to-br from-emerald-900 via-emerald-950 to-zinc-950/50 text-[var(--accent-foreground)] hover:bg-[var(--accent-hover)]"
