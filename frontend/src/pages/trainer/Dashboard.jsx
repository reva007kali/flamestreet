...
export default function Dashboard() {
  ...
  const promoQuery = useQuery({
    ...
    queryFn: async () => (await api.get('/promo-banners', { params: { audience: 'trainer' } })).data.banners,
  })
  ...
  if (query.isError) return <div className="text-red-300">Failed to load dashboard.</div>
    ...
    <div className="space-y-4">
            ...
            : [
                {
                  ...
                  subtitle: 'Bagikan referral link dan lihat activity realtime.',
                ...
                },
                {
                  ...
                  className: 'bg-gradient-to-br from-emerald-950/40 to-zinc-950',
                },
              ]
        }
      />
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5 sm:p-6">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold">{user?.full_name ?? 'Trainer'}</div>
          ...
          <div className="shrink-0 text-right">
            ...
            <div className="text-xs text-zinc-400">
              Rp {Number(stats.total_points_rupiah ?? 0).toLocaleString('id-ID')}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          >
            ...
            <div className="mt-2 text-[11px] font-medium text-zinc-200">Dashboard</div>
          </Link>
          <Link
          ...
          >
            ...
            <div className="mt-2 text-[11px] font-medium text-zinc-200">My Members</div>
          </Link>
          <Link
            to="/trainer/points"
            className="rounded-2xl  border border-zinc-800/80 bg-zinc-900/50 p-4 text-center backdrop-blur hover:border-zinc-700"
          >
            ...
            <div className="mt-2 text-[11px] font-medium text-zinc-300">Points</div>
          </Link>
          <Link
            to="/trainer/articles"
            className="rounded-2xl border border-zinc-800/80 bg-zinc-900/50 p-4 text-center backdrop-blur hover:border-zinc-700"
          >
            ...
            <div className="mt-2 text-[11px] font-medium text-zinc-200">Articles</div>
          </Link>
          <Link
            to="/trainer/profile"
            className="rounded-2xl border border-zinc-800/80 bg-zinc-900/50 p-4 text-center backdrop-blur hover:border-zinc-700"
          >
            ...
            <div className="mt-2 text-[11px] font-medium text-zinc-200">Profile</div>
          </Link>
          <a
            href="#activity"
            className="rounded-2xl border border-zinc-800/80 bg-zinc-900/50 p-4 text-center backdrop-blur hover:border-zinc-700"
          >
            ...
            <div className="mt-2 text-[11px] font-medium text-zinc-200">Activity</div>
          </a>
        </div>
      </div>
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          ...
          <Link to="/trainer/articles" className="text-sm text-[var(--accent)] hover:text-[var(--accent-hover)]">
            View all
          </Link>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-4">
          ...
          <div className="mt-1 text-2xl font-semibold">{stats.total_members}</div>
        </div>
        <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-4">
          <div className="text-sm text-zinc-400">Month Earning (points)</div>
          <div className="mt-1 text-2xl font-semibold">{stats.month_earning_points}</div>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div id="recent-members" className="rounded-lg border border-zinc-800 bg-zinc-900 p-4">
          <div className="font-medium">Recent Members</div>
          <div className="mt-3 space-y-2">
            {(query.data.recent_members ?? []).length ? (
              query.data.recent_members.map((m) => (
                <div key={m.id} className="rounded border border-zinc-800 bg-zinc-950 p-3 text-sm">
                  <div className="text-zinc-200">{m.full_name}</div>
                  <div className="mt-1 text-xs text-zinc-500">@{m.username}</div>
                </div>
              ))
            ) : (
