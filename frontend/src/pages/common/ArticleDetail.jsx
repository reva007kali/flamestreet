...
export default function ArticleDetail({ basePath }) {
  ...
  if (query.isLoading) return <div className="text-sm text-zinc-400">Loading...</div>
  if (query.isError) return <div className="text-sm text-red-300">Failed to load article.</div>
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center justify-between">
        <Link to={`${basePath}/articles`} className="text-sm text-[var(--accent)] hover:text-[var(--accent-hover)]">
          Back
        </Link>
        <div className="text-xs uppercase tracking-widest text-zinc-500">{fmtDate(a.published_at ?? a.created_at)}</div>
      </div>
