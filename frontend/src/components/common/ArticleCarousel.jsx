...
export default function ArticleCarousel({ articles, basePath, intervalMs = 6000 }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/50 backdrop-blur">
      <div className="flex transition-transform duration-500 ease-out" style={{ transform: `translateX(-${index * 100}%)` }}>
        {items.map((a, i) => (
          <Link key={a.id ?? i} to={`${basePath}/articles/${a.slug}`} className="h-[180px] w-full shrink-0">
            <div className="relative h-full w-full overflow-hidden">
              {a.cover_image ? (
                <img src={a.cover_image} alt="" className="absolute inset-0 h-full w-full object-cover" />
              ) : (
              ...
              <div className="relative z-10 flex h-full w-full items-end p-5">
                <div className="max-w-[520px]">
                  ...
                  <div className="mt-1 line-clamp-2 text-base font-semibold text-white">{a.title}</div>
                </div>
              </div>
            </div>
          </Link>
