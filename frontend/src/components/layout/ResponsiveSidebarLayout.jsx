...
export default function ResponsiveSidebarLayout({
  ...
  return (
    <div className="flex min-h-screen bg-[#080808] text-zinc-100">
      {/* DESKTOP SIDEBAR */}
          ...
          <nav className="space-y-1">
            ...
            {navItems.map(({ to, label, Icon }) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={`h-4 w-4 shrink-0 transition-transform duration-200 group-hover:scale-110 ${isActive ? "text-current" : "text-zinc-500 group-hover:text-zinc-300"}`}
                  />
        ...
        <div className="mt-auto border-t border-zinc-800/40 p-4">
          ...
          >
            <div className="relative h-9 w-9 overflow-hidden rounded-full border border-zinc-800">
              ...
              {unreadCount > 0 && (
                <span className="absolute right-0 top-0 h-2.5 w-2.5 rounded-full border-2 border-zinc-950 bg-[var(--accent)]" />
              )}
            </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* MOBILE HEADER */}
        <header className="sticky w-full top-0 z-[1000] flex h-13 items-center gap-3 border-b border-zinc-800/40 bg-zinc-950/80 px-4 backdrop-blur-xl lg:hidden">
          <Link
            to={brandTo}
            className="flex items-center gap-2 active:scale-95 transition-transform"
          >
            <div className="min-w-0">
              <div className="truncate text-md font-bold text-white uppercase">
                {brand}
              </div>
            </div>
          </Link>
          ...
          <div className="ml-auto flex items-center gap-2">
            <button
              type="button"
              className="relative flex h-8 w-8 items-center justify-center rounded-full bg-zinc-700 text-zinc-300 active:bg-zinc-800 transition-colors"
              onClick={async () => {
                setNotifOpen(true);
                await requestDeviceNotificationPermission();
              }}
        ...
        {/* PAGE CONTENT */}
        <main className="flex-1 pb-[calc(env(safe-area-inset-bottom)+7rem)] lg:px-10 lg:py-8 lg:pb-10">
            ...
            {navItems.slice(0, 5).map(({ to, label, Icon }) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <Icon
                      className={`h-5 w-5 ${isActive ? "drop-shadow-[0_0_8px_rgba(var(--accent-rgb),0.5)]" : ""}`}
                    />
                  </div>
                  <span
                    className={`text-[10px] font-bold tracking-tight leading-none ${isActive ? "opacity-100" : "opacity-60"}`}
                  >
                    {label}
                  </span>
                  {isActive && (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {n.message && (
                      <div className="mt-1 text-[11px] leading-relaxed text-zinc-500 line-clamp-2">
                        {n.message}
                      </div>
                    )}
